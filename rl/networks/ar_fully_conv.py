...
from rl.pre_processing import is_spatial_action, NUM_FUNCTIONS, FlatFeature, FLAT_FEATURES

ACTION_FLAT = [
  FlatFeature(0,  features.FeatureType.CATEGORICAL, NUM_FUNCTIONS, 'fn_id'),
]
...
class ArFullyConv():
  ...
  def build(self, screen_input, minimap_input, flat_input, actions_input):
    ...
    actions_emb = self.embed_obs(actions_input[0][:, np.newaxis], ACTION_FLAT, self.embed_flat)
    broadcast_actions = self.broadcast_along_channels(actions_emb, size2d)
    # todo: functions has at most three args
    #       spatial arg has at most one non_spatial arg and one spatial arg before
    #       non_spatial arg has at most one non_spatial arg before
    #       fn has NUM_FUNCTIONS category and non_spatial args has len(actions.TYPES)-3 category
    #       concat emb features of fn types and non_spatial arg types with fc
    #       concat broadcast of above emb features and spatial arg output with state_out
    #       if not have corresponding previous arg, leave it all zeros
    args_out = dict()
    for arg_type in actions.TYPES:
      if is_spatial_action[arg_type]:
        arg_out = self.spatial_output(self.concat2d([state_out, broadcast_actions]))
      else:
        arg_out = self.non_spatial_output(tf.concat([fc, actions_emb], axis=1), arg_type.sizes[0])
